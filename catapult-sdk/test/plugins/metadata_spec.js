		...
		describe('supports account metadata', () => {
			...
			test.binary.test.addAll(getCodec(EntityType.accountMetadata), 60, () => ({
				buffer: Buffer.concat([
					...
					Buffer.of(0x03, 0x00),
					Buffer.of(0x10, 0x00),
					Buffer.of(0x6d, 0x65, 0x74, 0x61, 0x20, 0x69, 0x6e, 0x66, 0x6f, 0x72, 0x6d, 0x61, 0x74, 0x69, 0x6f, 0x6e)
				]),
		...
		describe('supports mosaic metadata', () => {
			...
			test.binary.test.addAll(getCodec(EntityType.mosaicMetadata), 68, () => ({
				buffer: Buffer.concat([
					Buffer.from(targetPublicKey), // key 32b
					Buffer.of(0xF2, 0x26, 0x6C, 0x06, 0x40, 0x83, 0xB2, 0x92),
					Buffer.of(0x93, 0x53, 0xBB, 0x24, 0x12, 0xB1, 0xFF, 0x36),
					Buffer.of(0x05, 0x00),
					Buffer.of(0x10, 0x00),
					Buffer.of(0x6d, 0x65, 0x74, 0x61, 0x20, 0x69, 0x6e, 0x66, 0x6f, 0x72, 0x6d, 0x61, 0x74, 0x69, 0x6f, 0x6e)
				]),
		...
		describe('supports mosaic metadata', () => {
			...
			test.binary.test.addAll(getCodec(EntityType.namespaceMetadata), 68, () => ({
				buffer: Buffer.concat([
					Buffer.from(targetPublicKey), // key 32b
					Buffer.of(0xF2, 0x26, 0x6C, 0x06, 0x40, 0x83, 0xB2, 0x92),
					Buffer.of(0xAA, 0x22, 0xC2, 0x32, 0x99, 0xBC, 0xDE, 0x63),
					Buffer.of(0x12, 0x00),
					Buffer.of(0x10, 0x00),
					Buffer.of(0x6d, 0x65, 0x74, 0x61, 0x20, 0x69, 0x6e, 0x66, 0x6f, 0x72, 0x6d, 0x61, 0x74, 0x69, 0x6f, 0x6e)
				]),
