...
describe('account link plugin', () => {
	...
	describe('register codecs', () => {
		...
		describe('supports account link transaction', () => {
			...
			test.binary.test.addAll(getCodecs()[EntityType.accountLink][1], 32 + 1, () => ({
		...
		describe('supports node key link transaction', () => {
			...
			test.binary.test.addAll(getCodecs()[EntityType.nodeKeyLink][1], 32 + 1, () => ({
				buffer: Buffer.concat([
					linkedPublicKey,
					Buffer.of(0x01)
				]),
		...
		describe('supports voting key link transaction v1', () => {
			const linkedPublicKey = Buffer.of(
				0x77, 0xBE, 0xE1, 0xCA, 0xD0, 0x8E, 0x6E, 0x48, 0x95, 0xE8, 0x18, 0xB2, 0x7B, 0xD8, 0xFA, 0xC9,
				0x77, 0xBE, 0xE1, 0xCA, 0xD0, 0x8E, 0x6E, 0x48, 0x95, 0xE8, 0x18, 0xB2, 0x7B, 0xD8, 0xFA, 0xC9,
				0x47, 0x0D, 0xB8, 0xFD, 0x2D, 0x81, 0x47, 0x6A, 0xC5, 0x61, 0xA4, 0xCE, 0xE1, 0x81, 0x40, 0x83
			...
			test.binary.test.addAll(getCodecs()[EntityType.votingKeyLink][1], 48 + 4 + 4 + 1, () => ({
				buffer: Buffer.concat([
					linkedPublicKey, // 48b
					startEpoch, // 4b
					endEpoch, // 4b
		...
		describe('supports voting key link transaction v2', () => {
			const linkedPublicKey = Buffer.of(
				0x77, 0xBE, 0xE1, 0xCA, 0xD0, 0x8E, 0x6E, 0x48, 0x95, 0xE8, 0x18, 0xB2, 0x7B, 0xD8, 0xFA, 0xC9,
				0x77, 0xBE, 0xE1, 0xCA, 0xD0, 0x8E, 0x6E, 0x48, 0x95, 0xE8, 0x18, 0xB2, 0x7B, 0xD8, 0xFA, 0xC9
			);
			...
			test.binary.test.addAll(getCodecs()[EntityType.votingKeyLink][2], 32 + 4 + 4 + 1, () => ({
				buffer: Buffer.concat([
					linkedPublicKey, // 32
					startEpoch, // 4b
					endEpoch, // 4b
		...
		describe('supports vrf key link transaction', () => {
			...
			test.binary.test.addAll(getCodecs()[EntityType.vrfKeyLink][1], 32 + 1, () => ({
				buffer: Buffer.concat([
					linkedPublicKey,
					Buffer.of(0x01)
				]),
